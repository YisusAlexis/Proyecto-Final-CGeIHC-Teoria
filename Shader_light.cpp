...
void Shader::CreateFromFiles(const char* vertexLocation, const char* fragmentLocation)
{
	...
	{
		printf("No se pudieron leer los shaders: %s | %s\n", vertexLocation, fragmentLocation);
		return;
	}
...
void Shader::CompileShader(const char* vertexCode, const char* fragmentCode)
{
	...
	{
		printf("Shader source nulo.\n");
		return;
	}

	if (strlen(vertexCode) == 0 || strlen(fragmentCode) == 0)
	{
		printf("Shader source vacio.\n");
		return;
	}
	...
	for (size_t i = 0; i < MAX_POINT_LIGHTS; i++)
	{
		...
		snprintf(locBuff, sizeof(locBuff), "pointLights[%d].base.color", (int)i);
	...
	for (size_t i = 0; i < MAX_SPOT_LIGHTS; i++)
	{
		...
		snprintf(locBuff, sizeof(locBuff), "spotLights[%d].base.base.color", (int)i);
